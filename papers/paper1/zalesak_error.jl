...
function compute_shape_err(file_path, t_start,t_len)
...
function compute_all_errors(data, t_start,t_len)
    ...
    for (scheme, res_dict) in data
        ...
        E_sample = compute_shape_err(res_dict[Nx_vals[1]], t_start, t_len)
        n_periods = length(E_sample)
        
        err = zeros(n_periods, n_res)

        for (j, Nx) in enumerate(Nx_vals)
            E = compute_shape_err(res_dict[Nx], t_start, t_len)
            err[:, j] .= E
        end
...
function plot_convergence(results;
    ...
    for (scheme, res) in results
        ...
        # PERIOD CURVE
        # -------------------------
        scatterlines!(ax, Nx, err[1,:],linestyle = get(linestyle_map, scheme, :solid),marker = get(marker_map, scheme, :utriangle),label = "$scheme_label")
