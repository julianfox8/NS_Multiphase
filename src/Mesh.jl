...
# Define struct to handle the mesh hierarchy
struct mg_mesh_struct
    mesh_lvls:: Vector{mesh_struct}
end
...
function init_mg_mesh(param,par_env)
    @unpack mg_lvl = param
    mesh_lvls = Vector{mesh_struct}(undef,mg_lvl)

    for l in 1:mg_lvl
        mesh_lvls[l] = mg_create_mesh(param,par_env,l)
    end

    mg_mesh = mg_mesh_struct(mesh_lvls)
